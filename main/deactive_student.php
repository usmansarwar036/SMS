...
<div class="container-fluid">
	...
	<div class="contentheader">
			<i class="icon-table"></i> Deactive Students
			<a style=" margin-right: 139px; float: right;"  title="Click to go to deactive Student" href="students.php"><button class="btn btn-primary btn-large"> Active Students</button> </a>
			</div>
			<ul class="breadcrumb">
			<li><a href="index.php">Dashboard</a></li> /
			<li class="active">Deactive Srudents</li>
			</ul>
...
<a  href="index.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM students where status = 'deactive'  and class != 'Free intenship'  and class != 'Paid intenship'   ");
				$result->execute();
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			...
			<th width=""> Class </th>
			<th width=""> Cell </th>
			<th width=""> Admittion Date </th>
			<th width=""> Complete Date </th>
			<th width=""> Certificate </th>
			<th width=""> Total fee </th>
			<th width=""> Remaining fee </th>
			<th width=""> Action </th>
		</tr>
	</thead>
	<tbody>
			...
			<?php
				...
				include('../connect.php');
				$result = $db->prepare("SELECT * FROM students where status = 'deactive' and class != 'Free intenship'  and class != 'Paid intenship' order by id desc ");
				$result->execute();
		...
		<!--	<a  href="#" id="<?php echo $row['id']; ?>" class="delbutton" title="Click To Delete"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Delete</button></a>   -->
			<a  title="Click to deactive the Student" href="save_active_student.php?id=<?php echo $row['sr']; ?>"><button class="btn btn-primary btn-mini"><i class="icon-ok"></i> Active</button> </a>
		</td>
			</tr>
			<?php
				}
